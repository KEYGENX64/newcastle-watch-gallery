...
export class AuthUtils {
    ...
    static isSignInUsingToken(token: string): boolean {
        // Return if there is no token
        if (!token || token === '') {
            return false;
        }
        ...
        var offsetSec: number = environment.offsetSec || 0;
        ...
        var difference_date = difference_seconds / 1000;

        if (difference_date < 0) {
            difference_date = -difference_date
        }

        return difference_date > offsetSec;
    }

    static payLoad(token: string): any {
        // Return if there is no token
        if (!token || token === '') {
            return true;
        }
