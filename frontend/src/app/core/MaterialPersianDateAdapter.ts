...
export class MaterialPersianDateAdapter extends DateAdapter<jalaliMoment.Moment> {
    ...
    constructor() {
        ...
        this.AppSettingService.appSetting$.subscribe(app => {
            ...
            super.setLocale(app.localeData);
        })
    }
    ...
    getMonthNames(style: "long" | "short" | "narrow"): string[] {
        switch (style) {
            case "long":
            case "short":
                return jalaliMoment.localeData(this.localeData).jMonths().slice(0);
            case "narrow":
                return jalaliMoment.localeData(this.localeData).jMonthsShort().slice(0);
        }
    }
    ...
    getDayOfWeekNames(style: "long" | "short" | "narrow"): string[] {
        switch (style) {
            case "long":
                return jalaliMoment.localeData(this.localeData).weekdays().slice(0);
            case "short":
                return jalaliMoment.localeData(this.localeData).weekdaysShort().slice(0);
            case "narrow":
    ...
    getFirstDayOfWeek(): number {
        return jalaliMoment.localeData(this.localeData).firstDayOfWeek();
    }
    ...
    clone(date: jalaliMoment.Moment): jalaliMoment.Moment {
        return date.clone().locale(this.localeData);
    }
    ...
    today(): jalaliMoment.Moment {
        return jalaliMoment().locale(this.localeData);
    }

    parse(value: any, parseFormat: string | string[]): jalaliMoment.Moment | null {
        if (value && typeof value === "string") {
            return jalaliMoment(value, parseFormat, this.localeData);
        }
        return value ? jalaliMoment(value).locale(this.localeData) : null;
    }
    ...
    deserialize(value: any): jalaliMoment.Moment | null {
        ...
        if (typeof value === "string") {
            ...
            date = jalaliMoment(value).locale(this.localeData);
        }
